...
6. **装机签名核对 + 设备状态**：
   - 装官替前 `apksigner verify --print-certs` 比对手机现装 `com.tencent.mm` cert 与固定 `ca421ec3`（= 共存 = 新构建）。
   - 手机 `com.tencent.mm` 若 clean 未打补丁且 cert ≠ `ca421ec3`（609b4b18 = `0fe4ff85`）= 正版官方微信，该机测不了官替（`-r` 跨签名失败、卸正版丢数据），官替验证用另一台空机；该机用共存 `com.tencent.mn`（`ca421ec3`）测。
