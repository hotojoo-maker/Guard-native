...
> 更新时间：2026-06-06（对齐 HOOKMAP：P20 搜索 v15.2 + 密群搜索 L1；P21 Layer0b；密友/密群导入 UI）
...
|:--:|------|-------|:--:|------|:----:|
| **W1** | B模块触发器 + 搜索 | **P20B/P20** | P20 搜索 ✅；P20B 🟡（功能侧不阻塞；KPI 轻采样已记录；P26 fresh-warm 已有证据，不重复） | — | Sonnet |
| **W2** | 朋友圈小红点 | **P21_朋友圈小红点** | ✅ Layer0b 已验；备用层不阻塞 | — | Sonnet |
...
**下一步**：
1. 功能开发继续推进到 P22 缺口：普通消息通知 + 铃声功能。
...
**状态**：Layer0b ✅ 已装机实证；Layer2/v18 tab 备用层保留，非主线阻塞项。

**下一步**：仅在需要时复验备用层，不改 D1/D2/D3 已验 hook。
...
**状态**：⬜ 未领。发版前仍需空白 LSPosed KPI 基线。

**下一步**：跑 `frida_stats.js` 空白基线；业务 Proto/网络采集按 P18 brief 另开。
...
```
W1 B触发+搜索 ─→ P22 普通消息通知/铃声 / P26C 搜索高亮
W2 P21        ─→ 仅保留 Layer2 / v18 tab badge 备用层复验
W3 P17        ─→ 会话过滤已验收，不作为新任务入口
W4 P18        ─→ 空白 LSPosed KPI 基线 / 离线资料库（仍待领）
```

**当前启动顺序**：
1. 优先继续功能侧：P22 普通消息通知 + 铃声功能。
2. W2 只做 P21 备用层复验，不再阻塞 Layer0b 主线。
3. W4/P18 仍是 KPI 基线债，发版前必须补。
...
| **P22** | **PushFilter 通知策略层** | 🟡 | 主拦截/来电/未读已收口；当前缺口收敛为普通消息通知 + 铃声功能，详见 `docs/P22_PushFilter_VoIP.md`。|
...
| **P_NF1** | **密友普通消息通知策略** | 🟡 | 当前缺口：普通消息通知链路与铃声功能；后台震动已有实证。|
| **P_NF2** | **铃声功能 / 前台消息声** | ⬜ | backlog，需实现可听见的 SOUND 档或复用微信原生播放方式。|
