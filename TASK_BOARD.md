...
| **P_CertConverge** | **证书唯一真源收敛（官替/共存各一把 + 硬闸）** | ✅ | 2026-06-30 已被 v2「一套配方」覆盖（官替/共存 release 共用 official jks→`e3e13a49`）；新真源 `docs/RELEASE_LINE_SSOT_发行线统一口径.md` v2 + `DECISION_LOG` D-019；原任务卡已删除收敛（SUPERSEDED）。|
