...
| **P_CF3** | **【待办·优化·高优先级】H→V 热切密友未按实时时间顺序显示** | ⬜ | 2026-05-29 发现（守护内核10）。现象：HIDDEN→VISIBLE 热切恢复后，密友会话**不在正确的时间位置**（未按最近消息时间排序，疑似按 cache 原序/追加在末尾插回）。属会话热切区（ConvFilter restore 链：`restoreToMvvmList` / `restoreAdapterGraphFromCache` / 80ms post-dedup，见 CONV_REFRESH_PROBLEM §十六~十七 + F-35）。**与 P_CF2 越界崩溃疑同源**（都是 restore 注入与 WeChat 原生有序列表/count 未对齐）。**要求**：修复排序 + count 一致性，**不得破坏当前 v28 热切**、不得违反 F-35 强制规则（禁跨 List identity / list-visited 双层强 dedup；dedup 必须 notify 后异步）。**门控**：State only；改前走授权检查官。装机后补 KPI。 |
