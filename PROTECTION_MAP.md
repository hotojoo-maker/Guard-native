...
### E3 改零钱（**未实施 · 风险评估见 dispatch 收尾报告**）
- 现状：⛔ 调研阶段触发反 frida（F-37 MallIndexUIv2 杀进程），未实装。
- dispatch 风险评估：金融数据伪造，数量级高于 E2，触法律红线（涉嫌信息网络诈骗工具/帮信）。
- 决策：⬜ 待用户拍板降级方案（B1 仅 debug + 测试 wxid / B2 改余额遮罩 / B3 整条砍入 REJECTED_OPT）。
- 接入前必走：`/guard-auth-review` 合规预审 → 产出金融数据风险声明 + 用户告知文案 + kill switch 路径。
