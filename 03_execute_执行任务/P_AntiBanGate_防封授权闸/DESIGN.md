...
> 建立：2026-06-21 / 修订：2026-06-22（C36：减法+黑盒化+三触发汇一出口；C36 续：三抽屉+信任大脑两出口+不续命惩罚+状态机/评价体系+wxid/device 绑定+周期外置+As-Is；C36 续2：**决策锁定**[A2 接/device×A2 修/改 vip 走 A/时钟接受/SO 下沉后期] + 实施步骤 §9A；**C72**：登录砸门 + 人性诱捕链 §4A，防封解体=1h/最多2h（非72h）、72h 归隐私）/ 状态：**⬜ 设计 only 决策已锁，未改任何代码**（落代码 = git 快照 + 按 §9A 步骤 + 装机回归）
...
- **C36 阈值**：`T_soft=1h`（起软引流）/ `T_kill=72h`（真散→封号）。⚠️ **C72 修订**：防封解体 = 没授权 1h、最多宽限 2h（**覆盖本行 72h**）；72h 归**隐私线**离线宽限、与防封无关（见 §4A）。
...
## 4A. 人性诱捕链 — 登录砸门 × 蜜罐 × 租约暗号（C72 续 · 2026-06-22 · 攻击者旅程编排）

> 用户定调（C72，2026-06-22）：**根据人性设计**——把"对方一定会做的本能动作"变成陷阱扳机。本节把已有机制（§2 诱饵 / §4.1 料锁 / §4.2 载荷 canary / §4.3 黑盒化 / §5 影子期不续命 / §5A 信任档租约）**编排成一条攻击者旅程**，不新建机制（减法）。
> ⚠️ **时机口径修订（覆盖 §2/§5 早先 C36 记法）**：**防封解体 = 没授权 1h、最多宽限 2h**（不是 72h）；**72h 是隐私功能的离线宽限，与防封无关**。两条线各自计时、互不连坐。

### 总纲：手机 = 槽位（哑端，C72 锁定）
- 手机不做任何判断，只干三件事：①按服务器给的周期上报 ②收服务器签名下发的状态 ③照做。
- 所有判定（封/解体/过期/信任档/租约形态）**全在服务器算、签名下发**；手机端改不了（答案不在手机）。
- 周期/阈值（防封 1-2h、上报节奏、影子期、信任分）一律服务器签名下发，客户端不硬编码、不知道规则（§5A 周期外置）。
- **上报节奏（C72 提案）**：装机 1min/次 → 10min 后 5min/次 → 1h 后 10min/次 → 递减（服务器加防抖）。⚠️ 1min 起步需过防封官 CONN 红线(0.5) 复核再锁。

### 两种弹窗（都从唯一弹窗源 RiskPromptController 出，红线#9）
| 弹窗 | 触发 | 行为 | 对谁 |
|---|---|---|---|
| **登录砸门**（新增 = 登录 Enforcer/砸门抽屉）| 没登录 / 没授权 | **关不掉**、强制输授权码 | 老实没付钱的——硬挡白嫖直接用 |
| **引流弹窗**（现成 `FunnelPrompt`）| 篡改 / 影子期引流 | **可关闭、让他用** | 破解者——软、麻痹他 |

### 人性诱捕链（攻击者本能 → 我方扳机）
1. **显眼的饵**：登录砸门做得**好搜、明显**（"不要太假"），破解者本能去删它。→ §2 防封诱饵
2. **删一个冒一个**：删掉 → 第二个蜜罐引流弹窗冒出来；他以为没删干净、继续动。→ §4.2 载荷 canary
3. **真防线很难搜 + 打乱顺序**：饵显眼，但**真机制（canary 比对、解料 key 派生、第二弹窗、解体逻辑）做成很难搜索、字段/顺序打乱混淆**，他删不到根。→ §4.3 黑盒化 + §6.8 字符串混淆
4. **删/改 = 自爆 or 中蜜罐**：删弹窗 → 稳定核心掺进解料 key → key 错 → 防封料解不出 → 散沙 → 被封（§4.2）；翻"假开关"（假 `isAntiBanReady=true`）→ 中蜜罐（§2）。
5. **服务器全知、默不作声**：SO 察觉篡改 → 服务器标【被篡改不信任】→ 不弹不引流、只发 **7 天短租约、一直只续短**。→ 触发线C 影子期 + §5A 信任档 tier
6. **租约暗号（黑盒化升级 · C72 新增精彩点）**：正常用户租约**逐步升级**（短→长，建信任）；被篡改者**永远平的一周、从不升级**。SO 据"租约形态从不逐步升长"**自己读出'被篡改'**——**信号藏在租约形态里、无明面 isBanned 开关可 hook**。→ §4.3 黑盒化
7. **装死放饵**：进此态后不弹、不出声、假装能用，**故意让他研究、甚至让他把配方弄到手**（短租约期内种子仍解料；配方按版本轮换、用不久传不开）。→ §4.1 料锁 + §5 影子期"让他以为破解成功"
8. **秋后算账**：他以为赢了、拿去改去卖 → **将来防封解体 + 延迟弹窗**砸下来 → 他与其客户一起被宿主判非官方、被平台封。→ 触发线C 不续命 + §4 canary + 延迟弹窗

**一句话**：删它=自爆，关它=中蜜罐，删干净=进"假装成功"的影子局，最后秋后算账。**对方每个本能动作都是扳机。**

### 可复用模式（C72：以后一直用）
"**显眼饵 + 难搜真防线(打乱混淆) + 删即自爆(canary) + 假开关蜜罐 + 影子期养着 + 租约暗号 + 延迟不续命**" = 一套**通用反破解人性诱捕模板**；新功能 / 新版本均可复用，不必每次重新发明。

### v1 客户端先行 vs S9 服务器（诚实边界）
- **v1 客户端先行**：登录砸门弹窗 + 显眼饵 + canary(删即自爆) + 假开关蜜罐 + SO 读租约 tier/形态 + 本地影子期兜底。
- **S9 服务器（后接，才完整）**：标【被篡改不信任】、发 7 天影子短租约、租约形态藏信号、延迟不续命、全程"默不作声"的导演——**服务器没接前只是本地兜底，完整导演要等 S9**。
- **诚实**：Frida 动态 hook 解料出参仍是高级威胁；靠短租约 + 换版本 + 设备绑定让"破解结果用不久、传不开"，非无敌（北极星：破解成本高 + 收益寿命短，不追求破不了）。
...
| 授权闸宽限(B·T_grace) | **没篡改、只没付款的真用户** | `LeaseClock` age | 未授权 | 给时间授权，T_soft=1h 软引流，**防封解体 1h/最多 2h（C72 修订，见 §4A）；72h 归隐私线** |
