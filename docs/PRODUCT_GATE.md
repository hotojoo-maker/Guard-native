...
| **密码入口** | 全局搜索框输满密码 | `SearchUnlock` | 默认 `111111` |
| **B 模块触发** | 摇一摇 / Home / 切后台… | 待实现 | 推进状态机 → HIDDEN |
...
| 响应状态 | **仅 `HIDDEN`** 时：密码正确 → `UNLOCKING` → `VISIBLE`（显形密友） |
| 显形态 / 乱输 | **不改变状态机**（查岗安全） |
...
- 密码是用户掌握的 **入口**，从隐藏态 **显形**；进隐藏靠 B 触发 / 启动恢复 / 其它产品入口（待统一）。
...
| B2+B3 切后台/Home（CLOSE_SYSTEM_DIALOGS）| **开** | 防查岗最基础，不可关 |
| B4 返回键 | **关** | 误触，按页面判断复杂 |
