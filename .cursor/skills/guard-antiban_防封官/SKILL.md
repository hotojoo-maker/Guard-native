...
> **防封落地打法（与网络安全官 / DESIGN 对齐，本角色只研究、不复写机制）**：成品 = 用 LSPatch 把模块（含 C++ 核心 `libguardcore.so`）**重新打包进原版（官方包）**；落地靠**中间程序「掐 `getPackageInfo`/c$p 咽喉、骑脖子」**——灌官方值 = 保号（号不被判异常）+ 借官方的眼睛读「签名变没变」= 抓破解。**root/环境不是重点**（§0 L1：root 一月零封、读 unlocked 不 kill）；硬键 = 签名。机制全文 = `P_AntiBanGate/DESIGN.md §0`；用词按 `CLAUDE.md` 定死词表（封号→封/账号异常、注入→重新打包；防破解/逆向直写）。
> ⚠️ 注：本 skill 的**描述/触发关键词**仍保留「封号/被封」等词（否则用户提这些词时本 skill 不触发）——定死词表只约束**正文与产出文档**，不动触发关键词。
...
4. **KPI = 出包前体检项（非日常阻塞红线）**：出包前 `frida_stats` 跑一次——环境类 vbs(38)/PROP(220) 我方零环境读取本就达标；密度类 normsg(5124)/CONN(0.5) 明显异常才查。日常健康看：**身份(签名) + 卡顿/性能 + 零新增行为面**。
...
3. KPI 实测 / 基线更新
4. 竞品新样本入库
5. **官方包热更新（待办，2026-06-22）**：官方包不改版本号、悄悄热更新，可能推新检测 / 改动 → 检测面漂移、hook 失效；侦察阶段虽已收尾，官方包一热更新即把本角色叫回重巡检。
...
2. 版本底座是否对齐当前微信版本：
3. 是否有竞品样本 / 跨版本佐证：
4. KPI：环境类 vbs/PROP 出包前体检无倒退？密度类 normsg/CONN 明显异常才查？
