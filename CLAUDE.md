...
<!-- 最后更新：2026-06-10（文档收敛：以 06-09 看板 + 代码为准）；8071 文档隔离；V↔H 热切 ✅；防撤回(C1)/来电拦截(CA) ✅ 装机；FAILURE_LOG 至 F-38 -->
...
试错层   ████████████ 100%   F-01~F-38 已验证失败方案归档 (FAILURE_LOG.md)
```
...
完整清单 → [`FAILURE_LOG.md`](./FAILURE_LOG.md)（至 F-38，最新：F-38 伪装订位坐标候选证伪 · F-37 防撤回 a2.b→jy0.t.f · F-36 来电拦截 6 条证伪）
...
```
本仓库内:
  ./PROTECTION_MAP.md               防破解/防盗版总账 + 上线前门控（注入隔离/蜜罐/阶段）
