...
- **F-43** **克隆宿主签名 bleed-through + LSPatch `-l 2` sigbypass 运行时返回宿主原始签名** → 即使 LSPatch `-k` 用 official jks (文件级 apksigner = `e3e13a49`)、运行时 `bindSigningCert` 读宿主 sourceDir 仍拿到宿主**原始**签名 (克隆宿主 = `a40da80a`，官方原版 = `0fe4ff85`，LSPatch 内置 debug = `ca421ec3`，取决于何种宿主) ≠ EXPECTED `e3e13a49` → registry scatter、A2 不装、"授权异常" — **修复 = 装机前先 `apksigner sign --ks signing/guard-native-official-release.jks` 把克隆宿主真正重签为 e3e13a49，再 LSPatch -k 同把 official jks** (D-030)；另一半 cert binding 改读宿主 sourceDir = D-027 (是必要前提、不可撤)。L1 双证据：① 共存原 `mn_clean_origin` 文件 `a40da80a`、LSPatch 出包文件 `e3e13a49`、装机 PID `[native] certBind=a40da80a + [A2SIG] not installed`；② 老 AI 修复后宿主先 official 重签、装机 PID 23735 `[native] certBind=e3e13a49 + [A2SIG] installed + recipeOk=true` (证据 `03_execute_执行任务/P_HotUpdateFreeze_官方热更新冻结/logs/coexist_verify_20260701.txt`)。**强制**：① 克隆宿主必先 apksigner 重签 e3e13a49 (D-030 流程必跑) ② cert binding 走宿主 sourceDir (D-027) ③ 出货前必跑 `verify_cert_chain` 四端硬闸 (待建)。`apksigner verify --print-certs` 只看文件级、**不**能证明运行时 cert binding，必须 logcat 实证。
- **F-44** SearchFilter 把废弃锚点 ss4.p 当主路径迁进 registry（源于代码一句过期注释「TRUE PRIMARY: ss4.p」；ss4.p 父容器 GONE、onBindViewHolder 0 命中）→ 加密死路径 = 演戏（真锚点仍明文 / 迁死路径致功能失效）；装机 L1 实证真路径 `f0.getView`（`[SF:gv]`×29）、ss4.p `[SF:ss4p]` 0 次，已回滚删死代码（2026-06-09）—— **铁律：迁任何 Filter 读 registry 前先三证对账（代码常量 + HOOK_MAP_8071 + 必要时 L1 装机日志）确认锚点今天真活跃、任一吃不准就停、禁止照过期注释/草稿迁；registry 为子集时（如 conv.list 当初缺 contact_fields f/a/b/c）删 fallback 前先补全核准。** 迁出自安全官 skill（2026-07-03 P91）
