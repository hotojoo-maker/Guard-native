...
| P21 朋友圈小红点 / 互动列表 | ✅ | 2026-06-09 收尾 | Layer0b 入口归零 + P21B `bm -> s9.f(Cursor)` 按 `talker` 过滤；WithAll/bm L1，rm 同路径覆盖，后续有场景再复验 |
...
- P20B/E2 KPI 对比本轮不作为发版前硬阻塞；如后续进入正式发布门控，再按质检重档补跑。
...
- **V3-T1 LSPatch 劫持 8.0.71 可行性** 🟡 用户装机口述实证（2026-06-08，待补 logcat/截图）
  ...
  - **关键限制（用户实证）**：改包签名 = 我们的（非腾讯），微信内支付（红包/转账/扫码）可用，但**不能跳转第三方支付**（第三方 App 调起微信支付 / 微信跳转第三方支付时校验签名失败）；这是改包重签的**固有限制，不可解决** → 需要第三方支付跳转的客户必须保留官方 → 见下方共存版需求。
...
- **动机（用户实证）**：劫持改包**不能跳转第三方支付**（签名变→第三方支付跳转校验拒，**不可解决**；微信内支付本身可用）。不愿卸官方、或需要第三方支付跳转的客户，必须官方+我们改包**并存**：官方负责第三方支付跳转，改包负责隐私。
...
- **首选技术路线（比竞品强）**：**静态改包名 + LSPatch 注入模块** = 独立包名共存 + 无虚拟化容器。我们用 LSPatch 注入模块（hook 逻辑独立、好维护、走主流生态保护色）；竞品（甜密友/蜘蛛密友"分身版"）多用容器/虚拟化（高检测面）→ 我们**反检测面更低** = 差异化卖点。
